...
const
  ...
  TIGR_4X* = 8.cint
  TIGR_RETINA* = 16.cint
  TIGR_NOCURSOR* = 32.cint
  TIGR_FULLSCREEN* = 64.cint
...
proc tigrWindow*(w: cint; h: cint; title: cstring; flags: cint): ptr Tigr {.
    cdecl, importc: "tigrWindow", header: vim.}
##  Creates an empty off-screen bitmap.

proc tigrBitmap*(w: cint; h: cint): ptr Tigr {.cdecl, importc: "tigrBitmap",
    header: vim.}
##  Deletes a window/bitmap.

proc tigrFree*(bmp: ptr Tigr) {.cdecl, importc: "tigrFree", header: vim.}
##  Returns non-zero if the user requested to close a window.

proc tigrClosed*(bmp: ptr Tigr): cint {.cdecl, importc: "tigrClosed",
                                        header: vim.}
##  Displays a window's contents on-screen and updates input.

proc tigrUpdate*(bmp: ptr Tigr) {.cdecl, importc: "tigrUpdate", header: vim.}
##  Called before doing direct OpenGL calls and before tigrUpdate.
##  Returns non-zero if OpenGL is available.

proc tigrBeginOpenGL*(bmp: ptr Tigr): cint {.cdecl, importc: "tigrBeginOpenGL",
    header: vim.}
##  Sets post shader for a window.
##  This replaces the built-in post-FX shader.

proc tigrSetPostShader*(bmp: ptr Tigr; code: cstring; size: cint) {.cdecl,
...
proc tigrSetPostFX*(bmp: ptr Tigr; p1: cfloat; p2: cfloat; p3: cfloat;
                    p4: cfloat) {.cdecl, importc: "tigrSetPostFX", header: vim.}
##  Drawing ----------------------------------------------------------------
##  Helper for reading pixels.
##  For high performance, just access bmp->pix directly.

proc tigrGet*(bmp: ptr Tigr; x: cint; y: cint): TPixel {.cdecl,
    importc: "tigrGet", header: vim.}
##  Plots a pixel.
##  Clips and blends.
##  For high performance, just access bmp->pix directly.

proc tigrPlot*(bmp: ptr Tigr; x: cint; y: cint; pix: TPixel) {.cdecl,
    importc: "tigrPlot", header: vim.}
##  Clears a bitmap to a color.
##  No blending, no clipping.

proc tigrClear*(bmp: ptr Tigr; color: TPixel) {.cdecl, importc: "tigrClear",
    header: vim.}
##  Fills a rectangular area.
##  No blending, no clipping.

proc tigrFill*(bmp: ptr Tigr; x: cint; y: cint; w: cint; h: cint; color: TPixel) {.
    cdecl, importc: "tigrFill", header: vim.}
##  Draws a line.
##  Start pixel is drawn, end pixel is not.
##  Clips and blends.

proc tigrLine*(bmp: ptr Tigr; x0: cint; y0: cint; x1: cint; y1: cint;
               color: TPixel) {.cdecl, importc: "tigrLine", header: vim.}
##  Draws an empty rectangle.
##  Drawing a 1x1 rectangle yields the same result as calling tigrPlot.
##  Clips and blends.

proc tigrRect*(bmp: ptr Tigr; x: cint; y: cint; w: cint; h: cint; color: TPixel) {.
...
proc tigrFillRect*(bmp: ptr Tigr; x: cint; y: cint; w: cint; h: cint;
                   color: TPixel) {.cdecl, importc: "tigrFillRect", header: vim.}
##  Draws a circle.
...
proc tigrCircle*(bmp: ptr Tigr; x: cint; y: cint; r: cint; color: TPixel) {.
    cdecl, importc: "tigrCircle", header: vim.}
##  Fills a circle.
...
proc tigrFillCircle*(bmp: ptr Tigr; x: cint; y: cint; r: cint; color: TPixel) {.
    cdecl, importc: "tigrFillCircle", header: vim.}
##  Sets clip rect.
##  Set to (0, 0, -1, -1) to reset clipping to full bitmap.

proc tigrClip*(bmp: ptr Tigr; cx: cint; cy: cint; cw: cint; ch: cint) {.cdecl,
...
proc tigrBlit*(dest: ptr Tigr; src: ptr Tigr; dx: cint; dy: cint; sx: cint;
               sy: cint; w: cint; h: cint) {.cdecl, importc: "tigrBlit",
    header: vim.}
...
proc tigrBlitAlpha*(dest: ptr Tigr; src: ptr Tigr; dx: cint; dy: cint; sx: cint;
...
proc tigrBlitTint*(dest: ptr Tigr; src: ptr Tigr; dx: cint; dy: cint; sx: cint;
...
proc tigrBlitMode*(dest: ptr Tigr; mode: cint) {.cdecl, importc: "tigrBlitMode",
    header: vim.}
##  Helper for making colors.

proc tigrRGB*(r: uint8; g: uint8; b: uint8): TPixel {.inline, cdecl,
    importc: "tigrRGB".}
##  Helper for making colors.

proc tigrRGBA*(r: uint8; g: uint8; b: uint8; a: uint8): TPixel {.inline, cdecl,
    importc: "tigrRGBA".}
##  Font printing ----------------------------------------------------------
...
proc tigrLoadFont*(bitmap: ptr Tigr; codepage: cint): ptr TigrFont {.cdecl,
    importc: "tigrLoadFont", header: vim.}
##  Frees a font.

proc tigrFreeFont*(font: ptr TigrFont) {.cdecl, importc: "tigrFreeFont",
    header: vim.}
...
proc tigrPrint*(dest: ptr Tigr; font: ptr TigrFont; x: cint; y: cint;
                color: TPixel; text: cstring) {.varargs, cdecl,
    importc: "tigrPrint", header: vim.}
##  Returns the width/height of a string.

proc tigrTextWidth*(font: ptr TigrFont; text: cstring): cint {.cdecl,
    importc: "tigrTextWidth", header: vim.}
proc tigrTextHeight*(font: ptr TigrFont; text: cstring): cint {.cdecl,
...
proc tigrMouse*(bmp: ptr Tigr; x: pointer; y: pointer; buttons: pointer) {.
    cdecl, importc: "tigrMouse", header: vim.}
type
...
proc tigrTouch*(bmp: ptr Tigr; points: ptr TigrTouchPoint; maxPoints: cint): cint {.
...
proc tigrKeyDown*(bmp: ptr Tigr; key: TKey|cint): cint {.cdecl,
    importc: "tigrKeyDown", header: vim.}
proc tigrKeyHeld*(bmp: ptr Tigr; key: TKey|cint): cint {.cdecl,
    importc: "tigrKeyHeld", header: vim.}
##  Reads character input for a window.
##  Returns the Unicode value of the last key pressed, or 0 if none.

proc tigrReadChar*(bmp: ptr Tigr): cint {.cdecl, importc: "tigrReadChar",
    header: vim.}
##  Show / hide virtual keyboard.
##  (Only available on iOS / Android)

proc tigrShowKeyboard*(show: cint) {.cdecl, importc: "tigrShowKeyboard",
                                     header: vim.}
##  Bitmap I/O -------------------------------------------------------------
##  Loads a PNG, from either a file or memory. (fileName is UTF-8)
##  On error, returns NULL and sets errno.

proc tigrLoadImage*(fileName: cstring): ptr Tigr {.cdecl,
    importc: "tigrLoadImage", header: vim.}
proc tigrLoadImageMem*(data: pointer; length: cint): ptr Tigr {.cdecl,
    importc: "tigrLoadImageMem", header: vim.}
##  Saves a PNG to a file. (fileName is UTF-8)
##  On error, returns zero and sets errno.

proc tigrSaveImage*(fileName: cstring; bmp: ptr Tigr): cint {.cdecl,
...
proc tigrTime*(): cfloat {.cdecl, importc: "tigrTime", header: vim.}
##  Displays an error message and quits. (UTF-8)
##  'bmp' can be NULL.

proc tigrError*(bmp: ptr Tigr; message: cstring) {.varargs, cdecl,
